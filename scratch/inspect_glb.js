...
const file = path.join('f:', 'portfolio', 'public', 'blendertimer-low-poly-17.glb');
...
console.log(`Chunk 0 Type: ${chunkType}`);
...
if (gltf.meshes) {
  ...
  console.log('No meshes found in GLB.');
}

console.log('\n--- Materials ---');
if (gltf.materials) {
  gltf.materials.forEach((mat, index) => {
    console.log(`Material [${index}]: name="${mat.name}" pbrMetallicRoughness=${JSON.stringify(mat.pbrMetallicRoughness)}`);
  });
} else {
  console.log('No materials found in GLB.');
}
