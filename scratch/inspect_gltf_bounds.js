...
function inspectModel(filename) {
  ...
  // Find accessor min/max for POSITION attributes
  if (gltf.meshes) {
    gltf.meshes.forEach((mesh) => {
      ...
      mesh.primitives.forEach((prim, idx) => {
        ...
        if (posAccessorIdx !== undefined && gltf.accessors) {
          ...
          console.log(`  Primitive [${idx}]: POSITION min=${JSON.stringify(accessor.min)} max=${JSON.stringify(accessor.max)}`);
        }
      });
    });
  }
...
inspectModel('quaternius_cc0-boat-724.glb');
inspectModel('quaternius_cc0-torii-gate-983.glb');
